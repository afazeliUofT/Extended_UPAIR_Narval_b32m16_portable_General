#!/usr/bin/env bash
# Runtime probe for the standard normalized UMi link-level model.
# Run on a GPU node if possible; CPU also works but can be slow.
set -euo pipefail
...
print("[PROBE] channel.normalize_channel:", get_cfg(cfg, "channel.normalize_channel", None))
print("[PROBE] channel.umi:", get_cfg(cfg, "channel.umi", {}))
print("[PROBE] near_far:", get_cfg(cfg, "near_far", {}))

assert str(get_cfg(cfg, "channel.family", "")).lower() == "umi"
assert str(get_cfg(cfg, "channel.model", "")).lower() == "umi"
assert bool(get_cfg(cfg, "channel.normalize_channel", False)) is True
assert bool(get_cfg(cfg, "channel.umi.enable_pathloss", True)) is False
assert bool(get_cfg(cfg, "channel.umi.enable_shadow_fading", True)) is False
assert bool(get_cfg(cfg, "near_far.enabled", False)) is False

for u in [1, 2, 3, 4]:
    print("\n" + "="*100)
    print(f"[PROBE] Building normalized UMi system for U={u}")
    ...
    # Normalized UMi should not produce pathloss-level tiny powers around 1e-12.
    assert float(np.median(p)) > 1e-4, "median channel power is unexpectedly tiny; pathloss may still be active"
    if hasattr(channel, "last_near_far_stats"):
        assert not channel.last_near_far_stats, "near_far stats should remain empty when near_far.enabled=false"

# Estimator/feature path under U=4.
...
print("\n[PROBE] PASSED normalized UMi runtime channel/feature probe.")
PY
